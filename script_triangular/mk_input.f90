program main
  ...
  ! diagonal direction
  do i = 1, NOS-LX
    if(mod(i-1,LX) == 0)then
      write(20,'(2i8,9es23.15)') i, i+2*LX-1, JX, JY, JZ, DX, DY, DZ, GX, GY, GZ
    else
      write(20,'(2i8,9es23.15)') i, i+LX-1, JX, JY, JZ, DX, DY, DZ, GX, GY, GZ
    end if
  end do
  j = 1
  do i = NOS-LX+1, NOS
    if(mod(i-1,LX) == 0)then
      write(20,'(2i8,9es23.15)') i, LX, JX, JY, JZ, DX, DY, DZ, GX, GY, GZ
    else
      write(20,'(2i8,9es23.15)') i, j, JX, JY, JZ, DX, DY, DZ, GX, GY, GZ
      j = j + 1
    end if
